...
function configurePWAOptions(config) {
  ...
  if (pwa.pwaAssets) {
    const publicDir = typeof config.vite?.publicDir === "string" ? node_path.resolve(config.vite.publicDir) : node_path.resolve("public");
    pwa.pwaAssets.integration = {
      baseUrl: config.base ?? config.vite?.base ?? "/",
      publicDir,
      outDir: node_path.resolve(pwa.outDir)
    };
  }
...
function withUserConfig(config) {
  ...
  let api;
  vitePlugins.push(vitePluginPwa.VitePWA({ ...pwa }));
  vitePlugins.push({
