<template>
  <div class="menu-page">
    <n-card title="Menu Management" size="small">
      <n-form inline :model="filters" label-placement="left" label-width="auto">
        <n-form-item label="Name">
          <n-input v-model:value="filters.name" placeholder="Search by name" clearable />
        </n-form-item>
      ...
      <div class="menu-toolbar">
        <n-button type="primary" @click="openCreate">New Menu</n-button>
      </div>
    ...
    <n-modal v-model:show="showModal" preset="dialog" :title="modalTitle" :mask-closable="false">
      <n-form ref="formRef" :model="form" :rules="rules" label-placement="top">
        <n-grid cols="2" x-gap="16" y-gap="8">
          <n-form-item-gi label="Name" path="name">
            <n-input v-model:value="form.name" placeholder="Menu name" />
          </n-form-item-gi>
          <n-form-item-gi label="Type" path="type">
            <n-select v-model:value="form.type" :options="typeOptions" />
          </n-form-item-gi>
          <n-form-item-gi label="Path" path="path">
            <n-input v-model:value="form.path" placeholder="/system/menu" />
          </n-form-item-gi>
          <n-form-item-gi label="Component" path="component">
            <n-input v-model:value="form.component" placeholder="/system/menu/list" />
          </n-form-item-gi>
          <n-form-item-gi label="Icon">
            <n-input v-model:value="form.icon" placeholder="icon name" />
          </n-form-item-gi>
          <n-form-item-gi label="Parent">
            <n-select
      ...
      <template #action>
        <n-space>
          ...
          <n-button type="primary" :loading="saving" @click="submitForm">Save</n-button>
        </n-space>
      </template>
    </n-modal>
  </div>
...
<script lang="ts" setup>
  ...
  import { createMenu, deleteMenu, getMenuList, updateMenu } from '#/api/sys/menu';
  ...
  const modalTitle = computed(() => (editingId.value ? 'Edit Menu' : 'Create Menu'));
  ...
  async function submitForm() {
    ...
    try {
      ...
      if (editingId.value) {
        await updateMenu(editingId.value, { ...payload, id: editingId.value });
      } else {
